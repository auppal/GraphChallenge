...
def compute_best_block_merge_wrapper(tup):
    ...
    return compute_best_block_merge(blocks, num_blocks, interblock_edge_count, block_partition, block_degrees,
                                    args.n_proposal, block_degrees_out, block_degrees_in, args)


def compute_best_block_merge(blocks, num_blocks, M, block_partition, block_degrees, n_proposal, block_degrees_out,
                             block_degrees_in, args):
...
def propose_node_movement_profile_wrapper(tup):
    ...
    propose_node_movement_profile_stats.append(
        cProfile.runctx("rc.append(propose_node_movement_wrapper(tup))", globals(), locals(),
                        filename="propose_node_movement-%d-%d.prof" % (mypid, cnt)))
    return rc[0]
...
def propose_node_movement_wrapper(tup):
    ...
    (num_blocks, out_neighbors, in_neighbors, vertex_num_out_neighbor_edges, vertex_num_in_neighbor_edges,
     vertex_num_neighbor_edges, vertex_neighbors, self_edge_weights) = syms['static_state']

    (update_id_shared, M_shared, partition_shared, block_degrees_shared, block_degrees_out_shared,
     block_degrees_in_shared, block_modified_time_shared) = syms['nodal_move_state']
    ...
    if update_id != update_id_shared.value:
        if update_id == -1:
            (partition, block_degrees, block_degrees_out, block_degrees_in) \
                = (shared_memory_to_private(i) for i in
...
def propose_node_movement_profile_sparse_wrapper(tup):
    ...
    cnt = len(propose_node_movement_profile_stats)
    propose_node_movement_profile_stats.append(
        cProfile.runctx("rc.append(propose_node_movement_sparse_wrapper(tup))", globals(), locals(),
                        filename="propose_node_movement-%d-%d.prof" % (mypid, cnt)))
    return rc[0]
    ...
    if update_id == -1:
        ...
        while True:
            try:
                ...
                if args.verbose > 2:
                    print("Worker pid %d rank %d move remote %d from block %d to block %d." % (
                    worker_pid, myrank, ni, r, s))

                partition, M = move_node(ni, r, s, partition,
                                         out_neighbors, in_neighbors, self_edge_weights, M,
                                         block_degrees_out, block_degrees_in, block_degrees, args)
            except queue_empty:
                break
...
def move_node(ni, r, s, partition, out_neighbors, in_neighbors, self_edge_weights, M, block_degrees_out,
              block_degrees_in, block_degrees, args):
...
dtype_to_ctype = {"float64": ctypes.c_double, "float": ctypes.c_double, "int64": ctypes.c_int64, "int": ctypes.c_int,
                  "bool": ctypes.c_bool, "int32": ctypes.c_int32, "float32": ctypes.c_int32}
...
def nodal_moves_sequential(batch_size, max_num_nodal_itr, delta_entropy_moving_avg_window, delta_entropy_threshold,
                           overall_entropy_cur, partition, M, block_degrees_out, block_degrees_in, block_degrees,
                           num_blocks, out_neighbors, in_neighbors, N, vertex_num_out_neighbor_edges,
                           vertex_num_in_neighbor_edges, vertex_num_neighbor_edges, vertex_neighbors, self_edge_weights,
                           verbose, args):
...
def nodal_moves_parallel(n_thread_move, batch_size, max_num_nodal_itr, delta_entropy_moving_avg_window,
                         delta_entropy_threshold, overall_entropy_cur, partition, M, block_degrees_out,
                         block_degrees_in, block_degrees, num_blocks, out_neighbors, in_neighbors, N,
                         vertex_num_out_neighbor_edges, vertex_num_in_neighbor_edges, vertex_num_neighbor_edges,
                         vertex_neighbors, self_edge_weights, verbose, args):
    global syms
    ...
    syms['nodal_move_state'] = (
    update_id_shared, M_shared, partition_shared, block_degrees_shared, block_degrees_out_shared,
    block_degrees_in_shared, block_modified_time_shared)
    syms['args'] = args
        ...
        while proposal_cnt < N:
            ...
            for ni in range(start, stop, step):
                ...
                if verbose > 3 and accept:
                    print("Parent accepted %d result from worker %d to move index %d from block %d to block %d" % (
                    accept, rank, ni, partition[ni], s))
                ...
                if num_nodal_moves >= next_batch_cnt or proposal_cnt == N:
                    ...
                    if lock.acquire(block=block):
                        ...
                        if args.verbose > 2:
                            print("Modified fraction of id %d is %s" % (
                            update_id_cnt, len(where_modified) / float(M.shape[0])))
...
def entropy_for_block_count(num_blocks, num_target_blocks, delta_entropy_threshold, M, block_degrees, block_degrees_out,
    ...
    if n_thread_merge > 0:
        ...
        for current_blocks, best_merge, best_delta_entropy, fresh_proposals_evaluated in pool.imap_unordered(
                compute_best_block_merge_wrapper, [((i,), num_blocks) for i in L]):
        ...
        current_blocks, best_merge, best_delta_entropy, fresh_proposals_evaluated \
            = compute_best_block_merge(range(num_blocks), num_blocks, M,
                                       block_partition, block_degrees, args.n_proposal, block_degrees_out,
                                       block_degrees_in, args)
    ...
    for i, t in enumerate(num_target_blocks):
        ...
        state_per_num_blocks[i] = (
        overall_entropy, partition_t, num_blocks_t, M_t, block_degrees_out_t, block_degrees_in_t, block_degrees_t)
    ...
    if len(num_target_blocks) > 1:
        ...
        if len(num_target_blocks) == 3 \
            ...
            if verbose:
                print("Optimal stopping criterion found at %d blocks derivative %s at time %4.4f." % (
                num_target_blocks[1], dS_dn, timeit.default_timer() - t_prog_start))
            ...
            if verbose:
                print(
                    "Stopping criterion not found at %s blocks extrapolate to %s blocks derivative %s at time %4.4f." % (
                    num_target_blocks[1], extrapolated_newton, dS_dn, timeit.default_timer() - t_prog_start))
    ...
    (overall_entropy, partition, num_blocks_t, M, block_degrees_out, block_degrees_in, block_degrees) = \
    state_per_num_blocks[best_idx]
    ...
    if verbose:
        ...
        # total merging time & merging time at each round
        merge_time += timeit.default_timer() - t_prog_start - t_begin_merging
    ...
    if n_thread_move > 0:
        total_num_nodal_moves_itr, M = nodal_moves_parallel(n_thread_move, batch_size, args.max_num_nodal_itr,
                                                            args.delta_entropy_moving_avg_window,
                                                            delta_entropy_threshold, overall_entropy, partition, M,
                                                            block_degrees_out, block_degrees_in, block_degrees,
                                                            num_blocks, out_neighbors, in_neighbors, N,
                                                            vertex_num_out_neighbor_edges, vertex_num_in_neighbor_edges,
                                                            vertex_num_neighbor_edges, vertex_neighbors,
                                                            self_edge_weights, verbose, args)
    else:
        total_num_nodal_moves_itr, M = nodal_moves_sequential(batch_size, args.max_num_nodal_itr,
                                                              args.delta_entropy_moving_avg_window,
                                                              delta_entropy_threshold, overall_entropy, partition, M,
                                                              block_degrees_out, block_degrees_in, block_degrees,
                                                              num_blocks, out_neighbors, in_neighbors, N,
                                                              vertex_num_out_neighbor_edges,
                                                              vertex_num_in_neighbor_edges, vertex_num_neighbor_edges,
                                                              vertex_neighbors, self_edge_weights, verbose, args)
    ...
    if verbose:
        ...
        move_time = move_end_time - move_start_time # include merging time too
...
def load_graph_parts(input_filename, args):
    ...
    if args.parts >= 1:
        ...
        out_neighbors, in_neighbors, N, E, true_partition = load_graph(input_filename,
                                                                       load_true_partition=true_partition_available,
                                                                       strm_piece_num=1)
        for part in range(2, args.parts + 1):
            print('Loading partition {} of {} ({}) ...'.format(part, args.parts,
                                                               input_filename + "_" + str(part) + ".tsv"))
            out_neighbors, in_neighbors, N, E = load_graph(input_filename, load_true_partition=False,
                                                           strm_piece_num=part, out_neighbors=out_neighbors,
...
def find_optimal_partition(out_neighbors, in_neighbors, N, E, self_edge_weights, args, stop_at_bracket=False, verbose=0,
                           alg_state=None, num_block_reduction_rate=0.50, min_number_blocks=0):
    if verbose > -1:
    ...
    if not alg_state:
        ...
        overall_entropy = compute_overall_entropy(interblock_edge_count, block_degrees_out, block_degrees_in,
                                                  num_blocks, N, E)
        ...
        (hist, num_blocks, overall_entropy, partition, interblock_edge_count, block_degrees_out, block_degrees_in,
         block_degrees, golden_ratio_bracket_established, delta_entropy_threshold, num_blocks_to_merge,
         optimal_num_blocks_found, n_proposals_evaluated, total_num_nodal_moves) = alg_state
    ...
    while not optimal_num_blocks_found:
        ...
        if verbose:
            print("num_blocks = %s num_blocks_merged = %s M.shape = %s" % (
            num_blocks, num_blocks_merged, str(interblock_edge_count.shape)))
        ...
        (old_partition, old_interblock_edge_count, old_block_degrees, old_block_degrees_out, old_block_degrees_in,
         old_overall_entropy, old_num_blocks) = hist

        if verbose:
            print('Overall entropy: {} Number of blocks: {} Proposals evaluated: {} Overall nodal moves: {}'.format(
                old_overall_entropy, old_num_blocks, n_proposals_evaluated, total_num_nodal_moves))
        ...
        if np.all(np.isfinite(old_overall_entropy)):
            ...
            if not golden_ratio_bracket_established:
                ...
                print("Golden ratio found at blocks %s at time %4.4f entropy %s" % (
                old_num_blocks, timeit.default_timer() - t_prog_start, old_overall_entropy))
    ...
    hist, num_blocks, overall_entropy, partition, interblock_edge_count, block_degrees_out, block_degrees_in,
    block_degrees, golden_ratio_bracket_established, delta_entropy_threshold, num_blocks_to_merge,
    optimal_num_blocks_found, n_proposals_evaluated, total_num_nodal_moves)
...
def find_optimal_partition_wrapper(tup):
    ...
    return find_optimal_partition(out_neighbors, in_neighbors, N, E, self_edge_weights, args, stop_at_bracket=True,
                                  verbose=min(0, args.verbose - 1))
...
def merge_two_partitions(M, block_degrees_out, block_degrees_in, block_degrees, partition0, partition1,
...
def naive_streaming(args):
    ...
    for part in range(1, args.parts + 1):
        ...
        t_elapsed_partition, partition = partition_static_graph(out_neighbors, in_neighbors, N, E, true_partition, args,
                                                                stop_at_bracket=0, alg_state=None)
        t1 = timeit.default_timer()
        t_part += (t1 - t0)
        t_all_parts += t_part
        ...
        print('Elapsed compute time for part %d is %f cumulative %f precision %f recall %f' % (
        part, t_part, t_all_parts, precision, recall))
...
def copy_alg_state(alg_state):
    ...
    alg_state_copy = (hist_copy, num_blocks_copy, overall_entropy_copy, partition_copy, interblock_edge_count_copy,
                      block_degrees_out_copy, block_degrees_in_copy, block_degrees_copy,
                      golden_ratio_bracket_established_copy, delta_entropy_threshold_copy, num_blocks_to_merge_copy,
                      optimal_num_blocks_found_copy, n_proposals_evaluated_copy, total_num_nodal_moves_copy)

    return alg_state_copy


def incremental_streaming(args):
    global node_move_update_batch_size
    ...
    for part in range(1, args.parts + 1):
        ...
        if part == 1:
            ...
            if alg_state:
                ...
                t0 = timeit.default_timer()
                hist = alg_state[0]
                (old_partition, old_interblock_edge_count, old_block_degrees, old_block_degrees_out,
                 old_block_degrees_in, old_overall_entropy, old_num_blocks) = hist
                ...
                for j in [0, 2, 1]:
                    ...
                    total_num_nodal_moves_itr, M = nodal_moves_parallel(n_thread_move, batch_size,
                                                                        args.max_num_nodal_itr,
                                                                        args.delta_entropy_moving_avg_window,
                                                                        delta_entropy_threshold, overall_entropy,
                                                                        partition, M, block_degrees_out,
                                                                        block_degrees_in, block_degrees, num_blocks,
                                                                        out_neighbors, in_neighbors, N,
                                                                        vertex_num_out_neighbor_edges,
        ...
        t_elapsed_partition, partition, alg_state = partition_static_graph(out_neighbors, in_neighbors, N, E,
                                                                           ...
                                                                           alg_state=alg_state,
                                                                           min_number_blocks=min_number_blocks)
        min_number_blocks /= 2
        ...
        t0 = timeit.default_timer()
        t_elapsed_partition, partition = partition_static_graph(out_neighbors, in_neighbors, N, E, true_partition, args,
                                                                stop_at_bracket=0, alg_state=alg_state_copy,
                                                                min_number_blocks=5)
        t1 = timeit.default_timer()
        t_part += (t1 - t0)
...
def do_main(args):
    global syms, t_prog_start, f1_score, precision_score, recall_score
    ...
    if args.parts <= 1:
        ...
        if not args.test_resume:
            t_elapsed_partition, partition = partition_static_graph(out_neighbors, in_neighbors, N, E, true_partition,
                                                                    args, min_number_blocks=min_number_blocks)
        else:
            print("")
...
def partition_static_graph(out_neighbors, in_neighbors, N, E, true_partition, args, stop_at_bracket=0, alg_state=None,
                           min_number_blocks=0):
    global syms, t_prog_start, partition_time

    if args.verbose > 1:
        from collections import Counter
        print("Overall true partition statistics:")
        print("[" + "".join(("%5d : %3d, " % (i, e,) for i, e in
                             sorted([(e, i) for (i, e) in Counter(true_partition).items()]))) + "]\n")
    ...
    if args.mpi:
        ...
        t_prog_start = timeit.default_timer()
        alg_state, M = find_optimal_partition(out_neighbors_piece, in_neighbors_piece, N_piece, E_piece,
                                              self_edge_weights, args, stop_at_bracket=False, verbose=args.verbose)
        ...
        if args.verbose > 1:
            for j, _ in enumerate(true_partitions):
                ...
                print("[" + "".join(("%5d : %3d, " % (i, e,) for i, e in
                                     sorted([(e, i) for (i, e) in Counter(true_partitions[j]).items()]))) + "]\n")
        ...
        alg_state, M_bracket = find_optimal_partition(out_neighbors, in_neighbors, N, E, \
                                                      ...
                                                      alg_state=alg_state, num_block_reduction_rate=0.50,
                                                      min_number_blocks=min_number_blocks)
    ...
    partition_time = t_elapsed_partition
...
nodal_move_time = 0
partition_time = 0
node_move_update_batch_size = 0
block_size = [-1] * 100
merging_times = [-1] * 100
nodal_move_times = [-1] * 100
entropy_index = 0
precision_score = 0
recall_score = 0
f1_score = 0
...
if __name__ == '__main__':
    ...
    parser.add_argument("--t-merge", type=int, required=False, default=44,
                        help="Configure number of threads for both agglomerative merge phase (overrides --threads).")
    parser.add_argument("--t-move", type=int, required=False, default=5,
    ...
    parser.add_argument("input_filename", nargs="?", type=str,
                        default="../../data/static/simulated_blockmodel_graph_500_nodes")
    ...
    print(nodal_move_time-merge_time)
    print(merge_time)
